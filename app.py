...
def setup_rag_chain():
    ...
    prompt = ChatPromptTemplate.from_template("""
        ...
        2. 기타 고려될 수 있는 관련 규정들 (상황의 경중에 따라 적용될 수 있는 대체 옵션)
        3. 최종 안내: 최종 적용은 상황에 대한 면밀한 검토와 학생생활교육위원회의 판단 후 내려진다는 안내 문구.

        규정 문장: {context}
        질문: {question}

        철저히 검색된 규정 문서에 근거해서만 답변하세요. 특히 벌점 및 상점은 검색된 문서 내용의 '표, 리스트, 목록'을 참고하여 정확한 점수를 찾아 비교하여 제시하세요.
    """)
